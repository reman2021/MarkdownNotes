...
- [FadeTop](http://www.fadetop.com/)（提醒休息）
...
- [Visual Studio Code](https://code.visualstudio.com/download)
	- [配置VSCode编译环境（Win）](配置VSCode编译环境（Win）.md)
- Matlab
- Origin

## 软件使用
### FadeTop
下载的是 Portable 便携版，直接解压即可使用。
#### 添加开机自启动
1. 创建 FadeTop.exe 的快捷方式
2. `Win+R`，输入`shell:startup`，回车打开对应目录
3. 将 FadeTop.exe 的快捷方式粘贴至该目录下
#### 背景自定义
1. 将背景图放到 Settings.xml 所在目录下。
2. 在 Settings.xml 里找到`bg_image_file=""`，添加图片路径。